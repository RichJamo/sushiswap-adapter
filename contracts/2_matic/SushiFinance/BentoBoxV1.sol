...
import "../../utils/interfaces/BoringRebase.sol";
import "../../utils/interfaces/BoringERC20.sol";
import "../../utils/interfaces/BoringMath.sol";
import "../../utils/interfaces/BoringBatchable.sol";
import "../../utils/interfaces/MasterContractManager.sol";
import "../../utils/interfaces/IStrategy.sol";
import "../../utils/interfaces/IFlashBorrower.sol";
import "../../utils/interfaces/IBatchFlashBorrower.sol";
...
contract BentoBoxV1 is MasterContractManager, BoringBatchable {
    ...
    event LogDeposit(IERC20_ indexed token, address indexed from, address indexed to, uint256 amount, uint256 share);
    event LogWithdraw(IERC20_ indexed token, address indexed from, address indexed to, uint256 amount, uint256 share);
    event LogTransfer(IERC20_ indexed token, address indexed from, address indexed to, uint256 share);

    event LogFlashLoan(
        address indexed borrower,
        IERC20_ indexed token,
        uint256 amount,
        uint256 feeAmount,
        address indexed receiver
    );

    event LogStrategyTargetPercentage(IERC20_ indexed token, uint256 targetPercentage);
    event LogStrategyQueued(IERC20_ indexed token, IStrategy indexed strategy);
    event LogStrategySet(IERC20_ indexed token, IStrategy indexed strategy);
    event LogStrategyInvest(IERC20_ indexed token, uint256 amount);
    event LogStrategyDivest(IERC20_ indexed token, uint256 amount);
    event LogStrategyProfit(IERC20_ indexed token, uint256 amount);
    event LogStrategyLoss(IERC20_ indexed token, uint256 amount);
    ...
    IERC20_ private immutable wethToken;

    IERC20_ private constant USE_ETHEREUM = IERC20_(0);
    ...
    mapping(IERC20_ => mapping(address => uint256)) public balanceOf;

    // Rebase from amount to share
    mapping(IERC20_ => Rebase) public totals;

    mapping(IERC20_ => IStrategy) public strategy;
    mapping(IERC20_ => IStrategy) public pendingStrategy;
    mapping(IERC20_ => StrategyData) public strategyData;

    // ******************* //
    // *** CONSTRUCTOR *** //
    // ******************* //

    constructor(IERC20_ wethToken_) public {
    ...
    function _tokenBalanceOf(IERC20_ token) internal view returns (uint256 amount) {
    ...
    function deposit(
        ...
        // Effects
        IERC20_ token = token_ == USE_ETHEREUM ? wethToken : token_;
    ...
    function setStrategyTargetPercentage(IERC20_ token, uint64 targetPercentage_) public onlyOwner {
        // Checks
    ...
    function setStrategy(IERC20_ token, IStrategy newStrategy) public onlyOwner {
